  ...
  <div class="m-auto">
    <div class="pb-8">
      {#if connected && userBadges === null}
        <div>
          Loading ...<br /><br />If you have any problems, please contact us in
          the public Signal group
        </div>
      {:else}
        <div
        ...
        >
          {#each badges as badge}
            ...
            >
              ...
              <div class="w-full">
                {#if badge.shortname}<div class="uppercase font-bold">
                    {badge.shortname}
                  </div>{/if}
                <div class={badge.shortname ? "" : "font-bold"}>
                  <a
                    href={badge.url}
                    target="_blank"
                    class="external animate-section font-mono2 lowercase"
                    >{badge.name}</a
                  >
                </div>
                <div class="mt-4">
                  ...
                  <div class="pt-2">
                    ...
                    {:else if userBadges.badges[badge.id]?.eligible}
                      <button
                        ...
                        on:mouseenter={(ev) => animateText(ev, 15)}
                        >Eligible! Get discount!</button
                      >
                    {:else}
                      <button
                        class="button button-inactive text-gray-600 border-gray-600"
                        >Not eligible</button
                      >
                    {/if}
                  </div>
                {/if}
                {#if badge.text}
                  <div class="text-xs leading-6 pt-4 text-mild">
                    {badge.text}
                  </div>
                {/if}
              </div>
